...
if df_raw is not None:
    ...
    fig = make_subplots(
        ...
        subplot_titles=("1. 均線斜率與慣性門檻 (-2% ~ 2%)", f"2. 雙均線對照與過濾訊號 (左軸:{b_name} / 右軸:{l_name})"),
        row_heights=[0.3, 0.7],
    ...
    # 圖 2: 價格與雙均線
    # --- 左軸：原型 ETF ---
    fig.add_trace(go.Scatter(x=df.index, y=df["Base"], name=f"{b_name} 價", line=dict(color='blue', width=1.2, opacity=0.4)), row=2, col=1, secondary_y=False)
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df["Lev"], name=f"{l_name} 價", line=dict(color='red', width=1.2, opacity=0.4)), row=2, col=1, secondary_y=True)
    ...
    st.markdown("---")
    st.info(f"💡 **圖表更新說明**：現在圖中補上了 **{l_name} 的紅虛線均線** (右軸)。即使信號是由原型 ETF 的慣性決定，你依然可以觀察 {l_name} 本身是否回測其專屬的 200SMA 支撐點。")
