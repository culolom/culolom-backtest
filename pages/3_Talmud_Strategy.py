...
# 【修改點】按鈕不使用 use_container_width=True，維持小按鈕
if st.button("開始回測 🚀", type="primary"):
    with st.spinner("正在執行塔木德策略運算..."):
        # 1. 讀取數據
        ...
        for i, d in enumerate(dates):
            ...
            if rebalance_freq == "每年 (Yearly)":
                if i > 0 and d.year != dates[i-1].year: do_rebalance = True
            elif rebalance_freq == "每季 (Quarterly)":
                if i > 0 and d.quarter != dates[i-1].quarter: do_rebalance = True
            ...
            history_weights.append([holdings["RE"]/total_equity, holdings["STK"]/total_equity, holdings["CASH"]/total_equity])
        ...
        with tab1:
            ...
            fig_eq.update_layout(template="plotly_white", height=450, hovermode="x unified", title_text=f"策略 vs {bench_label}", legend=dict(orientation="h", y=1.02, x=1, xanchor="right"))
            st.plotly_chart(fig_eq, use_container_width=True)

        with tab2:
            st.caption("觀察重點：當某一資產價格大漲導致權重過高，策略會在再平衡日將其賣出。")
            fig_w = go.Figure()
            ...
            fig_w.update_layout(template="plotly_white", height=400, yaxis=dict(tickformat=".0%", title="權重"), hovermode="x unified", legend=dict(orientation="h"))
