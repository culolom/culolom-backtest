###############################################################
# app.py — 0050 雙向乖離動態槓桿 (修改版：以乖離率平均為牛熊分界)
###############################################################
...
st.set_page_config(page_title="雙向乖離動態槓桿系統 (均值分界版)", page_icon="📈", layout="wide")
...
st.markdown("<h1 style='margin-bottom:0.1em;'>📊 動態槓桿系統 (乖離均值分界版)</h1>", unsafe_allow_html=True)
...
sma_window = col_p4.number_input("SMA 基線週期", 10, 240, 200, step=10, help="計算乖離率基準的均線天數")
bias_avg_window = 200 # 固定為近200天的乖離率平均

st.write("---")
st.markdown(f"ℹ️ **策略說明**：本版本使用 **「{bias_avg_window}日乖離率平均線」** 代替單純的SMA作為牛熊分界線。當股價高於此平均線時視為牛市，低於時視為熊市。")

position_mode = st.radio("初始狀態選擇", ["一開局就全倉", "空手起跑 (等待下次買入訊號)"], index=0, horizontal=True)

col_set1, col_set2 = st.columns(2)
with col_set1:
    with st.expander("📉 熊市模式：負乖離 DCA 加碼設定", expanded=True):
...
with col_set2:
    with st.expander("🚀 牛市模式：高位乖離套利減碼設定", expanded=True):
...
if st.button("啟動回測引擎 🚀"):
    # 抓取足夠的緩衝資料以計算長天期平均
    max_window = max(sma_window, bias_avg_window)
    start_buf = start - dt.timedelta(days=int(max_window * 2.5))
    ...
    # 3. 計算乖離率的 200 日平均
    df["AvgBias"] = df["Bias"].rolling(bias_avg_window).mean()

    # 4. 計算新的牛熊分界線價格 (將平均乖離率轉換回價格水平)
    # 分界線價格 = SMA * (1 + 平均乖離率)
    df["BullBearLine"] = df["MA"] * (1 + df["AvgBias"])

    # 移除計算過程中產生的 NaN，並截取回測區間
    df = df.dropna(subset=["MA", "AvgBias", "BullBearLine"]).loc[start:end]
    
    if df.empty: st.error("⚠️ 計算均線與乖離率平均後數據不足，請調整開始日期或縮短週期參數。"); st.stop()
    ...
    for i in range(1, len(df)):
        # 取得當前與前一日的價格、分界線、乖離率
        p, bb_line, bias_pct = df["Price"].iloc[i], df["BullBearLine"].iloc[i], df["Bias"].iloc[i] * 100
        p0, bb_line0 = df["Price"].iloc[i-1], df["BullBearLine"].iloc[i-1]
        
        if dca_cd > 0: dca_cd -= 1
        if arb_cd > 0: arb_cd -= 1
        sig = 0
        
        # 修改核心邏輯：比較價格與「乖離率平均分界線」
        if p > bb_line: # 牛市區間
            if can_buy:
                # 黃金交叉：價格向上突破分界線
                if p0 <= bb_line0: curr_pos, sig = 1.0, 1
                # 套利減碼機制 (依據絕對乖離率)
            ...
            else: curr_pos = 0.0 # 尚未進入可買入狀態
        else: # 熊市區間
            can_buy = True # 一旦跌破分界線，下次突破即可買入
            # 死亡交叉：價格向下跌破分界線
            if p0 > bb_line0: curr_pos, sig = 0.0, -1
            # DCA 加碼機制 (依據絕對乖離率)
    ...
    data_map = {
        f"<b>{ch_name}</b><br><small>策略(均值分界)</small>": [sl[0]*capital, sl[1], sl[2], sl[7], sl[3], sl[4], sl[5], (df["Signal"]!=0).sum()],
    ...
    fig_price = go.Figure()
    # 股價與基礎 SMA
    fig_price.add_trace(go.Scatter(x=df.index, y=df["Price"], name=f"{ch_name} 股價", line=dict(color="#636EFA", width=1.5)))
    fig_price.add_trace(go.Scatter(x=df.index, y=df["MA"], name=f"{sma_window}SMA (計算基準)", line=dict(color="#FFA15A", width=1, dash='dot'), opacity=0.6))
    
    # 新增：乖離率平均分界線 (新的牛熊分界)
    fig_price.add_trace(go.Scatter(x=df.index, y=df["BullBearLine"], name=f"乖離率平均線 (牛熊分界)", line=dict(color="#D900FF", width=2)))

    # 訊號標註 (包含買進、賣出、加碼、減碼)
    # 顏色映射: 1=買進(綠), -1=賣出(紅), 2=加碼(深綠圓點), 3=減碼(橘色菱形)
    colors = {
        1: ("買進 (建倉)", "#00C853", "triangle-up", 12), 
        -1: ("賣出 (清倉)", "#D50000", "triangle-down", 12), 
        2: ("DCA 加碼", "#2E7D32", "circle", 10), 
        3: ("套利減碼", "#FF9800", "diamond", 10)
    }
    for v, (label, color, symbol, size) in colors.items():
        pts = df[df["Signal"] == v]
        if not pts.empty: 
            fig_price.add_trace(go.Scatter(
                x=pts.index, y=pts["Price"], mode="markers", name=label, 
                marker=dict(color=color, size=size, symbol=symbol, line=dict(width=1, color="white"))
            ))
            
    fig_price.update_layout(template="plotly_white", yaxis=dict(title="價格"), height=550, hovermode="x unified", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    st.plotly_chart(fig_price, use_container_width=True)

    # --- 圖表 2: 資金曲線比較 (維持不變) ---
    st.markdown("### 💰 資金曲線比較")
    fig_eq = go.Figure()
    fig_eq.add_trace(go.Scatter(x=df.index, y=df["Equity_Strategy"]-1, name="策略 (LRS+DCA)", line=dict(width=3, color="#00D494")))
    fig_eq.add_trace(go.Scatter(x=df.index, y=df["Equity_BH"]-1, name="Buy & Hold", line=dict(color="#FF4D4F", dash='dash')))
    fig_eq.update_layout(template="plotly_white", yaxis=dict(tickformat=".0%", title="累積報酬率"), height=450, hovermode="x unified")
    st.plotly_chart(fig_eq, use_container_width=True)

    # --- 圖表 3: 乖離率走勢與平均 (新增) ---
    st.markdown(f"### 📊 乖離率走勢 ({sma_window}SMA基準)")
    fig_bias = go.Figure()
    
    # 每日乖離率
    fig_bias.add_trace(go.Scatter(x=df.index, y=df["Bias"], name="每日乖離率", line=dict(color="grey", width=1), opacity=0.4))
    # 乖離率平均線
    fig_bias.add_trace(go.Scatter(x=df.index, y=df["AvgBias"], name=f"{bias_avg_window}日乖離率平均", line=dict(color="#D900FF", width=2.5)))
    
    # 零軸參考線
    fig_bias.add_hline(y=0, line_dash="solid", line_color="black", opacity=0.2)

    fig_bias.update_layout(template="plotly_white", yaxis=dict(tickformat=".1%", title="乖離率"), height=450, hovermode="x unified")
    st.plotly_chart(fig_bias, use_container_width=True)

    st.caption("免責聲明：本工具僅供策略研究參考，過去績效不代表未來表現，投資必有風險。")
